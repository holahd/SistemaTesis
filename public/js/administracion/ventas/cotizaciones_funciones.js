...
function confirmarventa() {
  ...
  const digitoRegion = identificacion.substring(0, 2);

  // Validación de cédula ecuatoriana
  if (digitoRegion >= 1 && digitoRegion <= 24) {
    var ultimo_digito = identificacion.substring(9, 10);
    var pares = parseInt(identificacion.substring(1, 2)) + parseInt(identificacion.substring(3, 4)) + parseInt(identificacion.substring(5, 6)) + parseInt(identificacion.substring(7, 8));
    var numero1 = identificacion.substring(0, 1);
    var numero1 = (numero1 * 2);
    if (numero1 > 9) { var numero1 = (numero1 - 9); }


    var numero3 = identificacion.substring(2, 3);
    var numero3 = (numero3 * 2);
    if (numero3 > 9) { var numero3 = (numero3 - 9); }

    var numero5 = identificacion.substring(4, 5);
    var numero5 = (numero5 * 2);
    if (numero5 > 9) { var numero5 = (numero5 - 9); }

    var numero7 = identificacion.substring(6, 7);
    var numero7 = (numero7 * 2);
    if (numero7 > 9) { var numero7 = (numero7 - 9); }

    var numero9 = identificacion.substring(8, 9);
...
function descargarCotizacion() {
  const cotizacionId = $('#cotizacion_id').val();
  $.ajax({
    url: '../../../ajax/cotizacion-serv.php?op=rutaCotizacion',
    method: 'POST',
    data: { idcot: cotizacionId },
    success: function (response) {
      const r = JSON.parse(response);
      if (r.status === 'ok') {
        window.open("../../" + r.pdfPath, '_blank');
        ...
        Swal.fire('Error', 'No se pudo obtener la ruta del PDF: ' + r.message, 'error');
      }
    },
    error: function () {
      Swal.fire('Error', 'Falló la conexión con el servidor.', 'error');
